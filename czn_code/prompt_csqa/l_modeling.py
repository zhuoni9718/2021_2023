...
PRETRAINED_MODEL_ARCHIVE_MAP = {
    ...
    #'bert-base-uncased': "/home/lifuh/CommonsenseQaPlus/baselines/pytorch_pretrained_bert/models/bert-large-uncased-ft.tar.gz",
    ...
    'bert-large-uncased-ft':"/home/lifuh/CommonsenseQaPlus/baselines/pytorch_pretrained_bert/models/bert-large-uncased-ft.tar.gz",
}
...
class PreTrainedBertModel(nn.Module):
    ...
    def from_pretrained(cls, pretrained_model_name, state_dict=None, cache_dir=None, *inputs, **kwargs):
        ...
        Params:
            ...
            state_dict: an optional state dictionnary (collections.OrderedDict object) to use instead of Google
            pre-trained models
...
class BertModel(PreTrainedBertModel):
    ...
    Inputs:
        `input_ids`: a torch.LongTensor of shape [batch_size, sequence_length]
            ...
            `extract_features.py`, `run_classifier.py` and `run_squad.py`), 词片嵌入
        `token_type_ids`: an optional torch.LongTensor of shape [batch_size, sequence_length] with the token
            types indices selected in [0, 1]. Type 0 corresponds to a `sentence A` and type 1 corresponds to
            a `sentence B` token (see BERT paper for more details).，句子嵌入
...
# Bert Transformer model + masked language model
...
# Bert Transformer model + next sentence prediction
...
# Bert Transformer model + sequence classification
...
# BERT Transformer with a multiple choice head on top
...
# BERT Transformer with a token classification head on top
...
class BertMultiwayMatch(PreTrainedBertModel):
    ...
    Inputs:
        ...
        `attention_mask`: an optional torch.LongTensor of shape [batch_size, num_choices, sequence_length] with indices
            ...
            input sequence length in the current batch. It's the mask that we typically use for attention when
            a batch has varying length sentences.
        `doc_length`, `ques_len`, `option_len`: option torch.LongTensors of shape [batch_size]. It's used to get the
            particular part of passage, question and option after bert encoding.
        `labels`: labels for the classification output: torch.LongTensor of shape [batch_size]
            with indices selected in [0, ..., num_choices].
